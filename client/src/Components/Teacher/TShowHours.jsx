...
//                         // hoursFull
//                         ...prev,
//                         [props.date.toLocaleDateString()]: HourRes.data.hoursEmpty,
//                     }));
//                 }
//             } catch (e) {
...
const TShowHours = (props) => {
        ...
        const fetchSelectedHours = async () => {
            try {
                ...
                if (HourRes.status === 200) {
                    ...
                    setSelectedHoursByDate((prev) => ({
                        ...prev,
                        [props.date.toLocaleDateString()]: { hoursFull, hoursEmpty },
                    }));
                }
            } catch (e) {
                if (e.response?.status === 404) {
    ...
    const handleHourClick = async (hour) => {
        ...
        const currentSelection = selectedHoursByDate[currentDate.toLocaleDateString()] || { hoursFull: [], hoursEmpty: [] };
    ...
    const currentDate = props.date ? props.date.toLocaleDateString() : null;
    const selectedHoursForCurrentDate = currentDate ? selectedHoursByDate[currentDate] || { hoursFull: [], hoursEmpty: [] } : { hoursFull: [], hoursEmpty: [] };
